...
Device <- setRefClass(
  ...
  methods = list(
    ...
    should.consider.rec = function(id.sender, transaction.num) {
      ...
      return(
        recced.trust > (params$delta.a - params$trust.rep.adj.range) |
          sapply(
            1:length(transaction.num),
            function(i) {
              acceptable.rec(
                cached.contexts[[id.sender]],
                contexts[[id.sender]][get.context.index(transaction.num[[i]])],
                recced.trust[[i]]
              )
            }
          )
    ...
    transaction = function(devices) {
      ...
      rs.dir.trust <- find.direct.trust(normalized.c.target)
      used.trust <- `if`(
        abs(rs.dir.trust$trust.comb) <=
          (params$trust.rep.threshold + params$trust.rep.adj.range),
        find.indirect.trust(normalized.c.target),
    ...
    find.direct.trust = function(normalized.c.target) {
      ...
      return(
        list(
          ...
          trust.comb = dir.trust
        )
      )
    },

    find.indirect.trust = function(normalized.c.target) {
